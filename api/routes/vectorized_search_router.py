...
from api.dtos.requests_dtos import (
    VectorizedSearchPreviewPayload,
)  # Asegúrate de definir este DTO
...
@vectorized_search_router.post(
    "/", response_model=VectorizedSearchResponse, response_model_exclude_none=True
)
async def vectorized_search(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred during vectorized search"
        )
