...
class BertopicRepository:
    ...
    def __calculate_topics(
        self, content_list: typing.List[str], created_at_list: typing.List[str]
    ):
    ...
    async def get_topics(
        ...
        for i in topic_size.keys():
            if 0 <= i < num_topics:
                topic_pie_slices.append(
                    PieSlice(name=f"topic_{i}", value=topic_size[i], color="grey")
                )
        topic_plots.append(
            PieChart(
                category="topics",
        ...
        for topic in range(num_topics):
            ...
            df_filtered = topics_over_time.loc[
                (topics_over_time["Topic"] == topic)
            ].sort_values("Timestamp", ascending=False)

            topic_docs = doc_info[doc_info["Topic"] == topic].sort_values(
            ...
            topic_barplot = Barplot(
                ...
                bars=[
                    BarPlotUnit(name=word, value=float(score))
            ...
            topic_wordcloud = Wordcloud(
                category=f"topic_{topic}",
                data=[
                    Word(name=word, frequency=int(float(score) * 10))
                    for word, score in valid_words
                ],
            )
            ...
            for _, row in df_filtered.iterrows():
                time_index = time_serie_data.index(
                    row["Timestamp"].strftime("%d %b %H:%M")
                )
                serie_data[time_index] += row["Frequency"]
            topic_timeline_series.append(
                StackedSerie(name=f"topic_{topic}", data=serie_data)
            )
        ...
        topics_inference_result = (
            await self.llm_repository.create_topics_name_and_summary(
                inference_data=topics_data_for_inference, num_keywords=8, num_docs=8
            )
        )
        ...
        for topic in range(num_topics):
            ...
            if topic in topics_inference_result:
                ...
                description = (
                    f"Documento Representativo: {docs_list[0]}" if docs_list else ""
                )
            topic_info = TopicInfo(
                topic=f"topic_{topic}", title=name, summary=description
            )
            topic_plots.append(topic_info)

        topic_stackline = StackedLine(
            title="topics", x=time_serie_data, series=topic_timeline_series
        )
