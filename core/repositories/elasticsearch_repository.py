...
class ElasticsearchRepository:
    ...
    def __init__(
        self, elasticsearch_service: ElasticsearchService, page_size: int = 1000
    ):
    ...
    async def get_aggs_data(self, index_pattern: str, query: Query) -> typing.Dict:
        ...
        response = await self.elasticsearch_service.run_search_query(
            index_pattern=index_pattern, query=query.body
        )

        aggs_data = defaultdict(dict)
        for agg_name, agg_result in response.aggregations.items():
            aggs_data[agg_name] = {
                item["key"].lower(): aggs_data[agg_name].get(item["key"].lower(), 0)
                + item["doc_count"]
                for item in agg_result["buckets"]
                if item["key"]
            }
    ...
    async def get_index_data(
        self, index_pattern: str, body: dict
    ) -> typing.Tuple[typing.List[Document], typing.List]:
        search_results = await self.elasticsearch_service.run_search_query(
            index_pattern=index_pattern, query=body
        )
        if not search_results.hits:
            return ([], [])
        documents = [Document(**doc) for doc in search_results.hits]
        return documents, search_results.last_sort_id

    async def get_paginated_data(
        self, index_pattern: str, query: Query, max_ndocs: int = None
        ...
        while True:
            remaining_docs = (
                max_ndocs - len(total_hits) if max_ndocs else self.page_size
            )
            ...
            logger.info(
                f"{len(hits)} documents brought in the page number {i} from the index: {index_pattern}"
            )
            i += 1
        ...
        documents = [Document(**doc) for doc in total_hits]
        return documents
