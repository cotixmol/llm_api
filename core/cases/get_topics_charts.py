...
import iso8601
from core.repositories.bertopic_repository import (
    BertopicRepository,
    BertopicRepositoryException,
)
...
class GetTopicChartsCase:
    ...
    async def __call__(self) -> Tuple[Dict, int]:
        ...
        ### GET TOPICS ###
        created_at_list, content_list, embedding_list = self.__prepare_data(
            documents_list
        )

        if not all((content_list, embedding_list)):
            raise BertopicRepositoryException(
                "There are no documents to calculate topics"
            )
