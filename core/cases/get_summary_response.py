...
class GetSummaryResponseCase:
    ...
    async def __call__(self) -> LLMSummaryResponse:
        ...
        if isinstance(self.query, str):
            self.query_repository.set_query_string(
                query_string=f'(NOT category.keyword: "Streaming") AND (NOT content_type.keyword: "Repost") AND ({self.query})'
            )
        else:
            self.query_repository.set_query_string(
                query_string='(NOT category.keyword: "Streaming") AND (NOT content_type.keyword: "Repost")'
            )
        ...
        try:
            ...
            if self.summary_field:
                ...
                response = await self.es_repository.get_aggs(
                    query=self.query_repository, index_pattern=self.index_pattern
                )
            else:
                # if not summary field set order of documents by interactions
                self.query_repository.set_sort(
                    "interactions", {"order": "desc", "unmapped_type": "long"}
                )
                ...
                if not response:
                    raise ElasticsearchException(
                        f"No documents found for index pattern: {self.index_pattern}"
                    )

            ### MAKE PREDICTION ###
            match (self.summary_field, self.query):
                case (
                    str() as summary_field,
                    _,
                ):  # Entra si summary_field es un str, sin importar query
                    response_dict = (
                        await self.llm_repository.apply_prompt_categories_summary(
                            aggs=response,
                ...
                case (
                    None,
                    str() as query,
                ):  # Entra solo si summary_field es None y query es un str
                    response_dict = (
                        await self.llm_repository.apply_prompt_query_summary(
